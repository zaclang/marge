...
export default class TeamBot {
  ...
  say(response) {
    const message = response.match[2] && response.match[2].trim();
    console.log(this.robot.adapter.client.chat);
    this.robot.adapter.client.chat.postMessage(response.user.room, message);
  }
