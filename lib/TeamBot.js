...
export default class TeamBot {
  ...
  async priceCheck(response) {
    ...
    return response.send(`${username}, The last price for ${CRYPTO_COIN} was $${lastPrice} (${CURRENCY})`);      
  }
  ...
  async delegate(response, suggestedReviewer) {
    const teamId = response.message.room;
    let nextReviewerName = suggestedReviewer || this.trimName(response.match[2]);
