...
export default class TeamBot {
  ...
  priceCheck(response, currency = 'eth') {
    return axios.get(`https://coinmarketcap-nexuist.rhcloud.com/api/${currency}`)
      .then(({ data }) => {
        const { price: { aud }} = data;
        return response.send(`Ethereum Price: $${aud} (AUD)`);
      });
  }
