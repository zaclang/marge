...
export default class TeamBot {
  ...
  skip(response) {
    return this.delegate(response, {requestorCommand: false});
  }

  rewind(response) {
    return this.delegate(response, {rewind: true, requestorCommand: false});
  }

  delegate(response, {rewind = false, requestorCommand = true}) {
    const members = this.robot.brain.get('members');
    const requestor = requestorCommand && response.message.user.name;
