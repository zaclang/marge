...
export default class TeamBot {
  ...
  delegate(response) {
    ...
    const newIndex = availableMembers.indexOf(lastReviewer) + 1;

    const reviewer = newIndex <= members.length
      ? availableMembers[newIndex]
      : availableMembers[0];

    response.send(`${reviewer}'s turn to review this PR!`);
