...
export default class TeamBot {
  ...
  delegate(response, {rewind = false, skip = false, username } = {}) {
    ...
    return Promise.all([
      ...
      this.store.getLastReviewer(teamId),
      this.store.getLastRequestor(teamId),
    ])
      ...
      .tap(({ reviewer, requestor }) => this.store.setLastReviewer(reviewer, teamId))
      .then(({ requestor }) => this.store.setLastRequestor(requestor, teamId))
      .catch(e => response.send(e.message));
  }
}
