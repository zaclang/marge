  ...
  delegate(response, {rewind = false, skip = false, username } = {}) {
    ...
    return Promise.all([
      ...
      .then(([members, lastReviewerName, lastRequesterName]) => {
        ...
        const requesterName = skip || rewind
          ? lastRequesterName
        ...
        if (rewind) {
          ...
          nextReviewerIndex = lastReviewerIndex  > -1
            ? lastReviewerIndex
            : members.length - 1;

          if (nextReviewerIndex == 0 && lastReviewerIndex == 0) {
            nextReviewerIndex = members.length - 1;
          }
        ...
        const responses = [
          ...
          `${requesterName ? requesterName + ` doesn't` : `we don't`} have all day!`,
          `${requesterName ? requesterName + ' is' : 'we are'} waiting...`,
          'STOP! PR time.',
